...
myDates$Voltage <- as.numeric(levels(myDates$Voltage))[myDates$Voltage]
myDates$Global_reactive_power <- as.numeric(levels(myDates$Global_reactive_power))[myDates$Global_reactive_power]
myDates$Global_active_power <- as.numeric(levels(myDates$Global_active_power))[myDates$Global_active_power]

# Create plot4
par(mfrow = c(2,2))
with(myDates, {
    plot(Calendar, Global_active_power, type = "l", xlab = "", ylab = "Global Active Power (kilowatts)")
    plot(Calendar, Voltage, type = "l", xlab = "datetime", ylab = "Voltage")
    ...
    plot(Calendar, Global_reactive_power, type = "l", xlab = "datetime", ylab = "Global reactive power")
})
